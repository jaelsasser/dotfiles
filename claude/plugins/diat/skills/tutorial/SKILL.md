...
- **Concrete over abstract.** Real commands, real filenames, real output. Placeholders break a beginner who can't yet fill them in.
- **Minimal explanation.** Defer the *why* to explanation-mode — a tutorial that stops to justify each step loses the thread. A one-clause aside is fine; a paragraph is a digression.
- **No alternatives, no edge cases, no digressions.** Cut everything that isn't on the single path to the win.

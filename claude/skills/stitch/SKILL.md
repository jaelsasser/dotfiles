...
allowed-tools: Bash(~/.claude/skills/stitch/scripts/stitch.sh *)
---

Your current session ID is `${CLAUDE_SESSION_ID}`. `/stitch` requires a **fresh** session — `stitch.sh take` refuses when source SID equals destination SID. Available seams from prior sessions:

!`${CLAUDE_SKILL_DIR}/scripts/stitch.sh list`
...
- If `$ARGUMENTS` is `--dry-run` or `dry-run`: identify which session would be chosen using the rules below, report its `sid:`, `head:`, and pending seam count, then stop — do not call `stitch.sh take`.
...
```
CLAUDE_SESSION_ID="${CLAUDE_SESSION_ID}" ${CLAUDE_SKILL_DIR}/scripts/stitch.sh take <source-sid>
```

Respond with the full output of `stitch.sh take` verbatim — the confirmation line, then the migrated seam content if any follows.

If `take` fails, surface its stderr verbatim. The three refuse cases are:

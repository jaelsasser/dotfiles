...
paths:
  ...
  - "**/hooks/*.md"
---
Conventions for editing the Claude-facing prose in the `claude` dotfiles package — `USER_CLAUDE.md` (symlinked to `~/.claude/CLAUDE.md`), command bodies, skill bodies, and hook output. The live files are ground truth: when a convention here lags what `USER_CLAUDE.md` actually does, update this rule to match, not the reverse.

`USER_CLAUDE.md` is paid for on every turn of every session; commands, skills, and hooks are paid for on demand. **This cost asymmetry drives most of the decisions below.**

...
chezmoi init --apply <repo-url>             # first install on a new host (prompts for git email)
./run-tests.sh               # bats + pytest, against a temp $HOME
```

## Architecture

See [AGENTS.md](AGENTS.md) for the source layout, chezmoi naming conventions, the live `claude/` symlink farm, the `settings.json` merge, externals, OS gating, and per-host data.

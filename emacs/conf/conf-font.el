...
;; self-removes; ligatures engage only for families that resolve them in Emacs.

(defvar my--coding-font-stack
...
(defun my--apply-coding-font (&optional frame)
  "Set the global coding font once a graphical FRAME exists; self-removing."
  (when (display-graphic-p frame)
    (let ((families (font-family-list frame)))
      (when-let* ((pick (seq-find (lambda (e) (member (car e) families))
                                  my--coding-font-stack)))
        ...
        (remove-hook 'after-make-frame-functions #'my--apply-coding-font)))))

(use-package ligature
  :config
  ;; One union set for prog-mode; the font decides which sequences have glyphs.
  ;; (my--apply-coding-font owns the global toggle, so don't enable it here.)
...
(add-hook 'after-make-frame-functions #'my--apply-coding-font)
(my--apply-coding-font)

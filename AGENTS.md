...
**Current state only — no cross-refs, no changelog.** A comment describes the code as it stands, never why a line changed (that's `git log`), nor the investigation that justified it (that's the commit message), nor a fact declared elsewhere in the config. Calibration: `; C-c c is the claude prefix` beside a keybind, when that map lives in `my-agent.el` → cut it. Calibration: `~8ms fork`, `no global config` — the benchmark that led you to the design → commit message; `inside || was` is the teardown — the line that reads as a bug without it → the one comment that earns its slot. The urge to memorialize every finding peaks right after the investigation; that's the tell, not the warrant.
